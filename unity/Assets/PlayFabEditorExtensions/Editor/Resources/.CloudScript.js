...
handlers.ListVMSummaries = function (args, context) {
    var VMSummaries = multiplayer.ListVirtualMachineSummaries({
        "BuildId": "756f263e-f6a1-46cd-a124-07d1a3ec128a",
        "Region": "NorthEurope", //Your region.
    });
    return VMSummaries;
};

handlers.ListServerSummaries = function (args, context) {
    var buildId = null;
    if (args && args.hasOwnProperty("buildId")) {
        buildId = args.buildId;
    }
    
    var region = "NorthEurope";
    if (args && args.hasOwnProperty("region")) {
        region = args.region;
    }
    var summaries = multiplayer.ListMultiplayerServers({
        "BuildId": buildId,
        "Region": region, 
    });
    
    var returnArray = [];
    for (var i = 0; i < summaries['MultiplayerServerSummaries'].length; i++) {
        if(summaries['MultiplayerServerSummaries'][i]['ConnectedPlayers'].length < 100) {
            returnArray.push(summaries['MultiplayerServerSummaries'][i]);
        }
    }
    var returnObject = {};
    returnObject['MultiplayerServerSummaries'] = returnArray;
    return returnObject;
};

handlers.MultiplayerServerDetails = function (args, context) {
    var buildId = null;
    if (args && args.hasOwnProperty("buildId")) {
        buildId = args.buildId;
    }
    ...
    var sessionId = null;
    if (args && args.hasOwnProperty("sessionId")) {
        sessionId = args.sessionId;
    }
    var details = multiplayer.GetMultiplayerServerDetails({
        "BuildId": buildId,
        "Region": region, 
        "SessionId": sessionId
    });
